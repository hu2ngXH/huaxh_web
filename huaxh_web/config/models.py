...
class Link(models.Model):
    ...
    weight = models.PositiveIntegerField(default=1, choices=zip(range(1, 6), range(1, 6)), verbose_name="权重",
                                         help_text="权重高展示顺序靠前")
    owner = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="作者")  # 一个作者可以有多种友链 一条友链只能针对一个作者
...
class SideBar(models.Model):
    ...
    owner = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="作者")  # 一个作者可以有多个侧边栏 但是一个侧边栏只能对应一个专属的作者
    created_time = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
